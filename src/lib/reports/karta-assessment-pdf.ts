...
export function downloadKartaAssessmentPdf(input: KartaAssessmentPdfInput): void {
  ...
  doc.setTextColor(...MUTED);
  doc.text(
    "Generated by Kharta — Confidential — Not for distribution",
    MARGIN,
    y,
  );
  doc.text(generatedLabel, PAGE_W - MARGIN, y, { align: "right" });

  const filename = `Kharta Assessment ${safeFilenamePart(input.candidateName)} ${formatFileDate(generated)}.pdf`;
  doc.save(filename);
}
