    ...
    <div className="space-y-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-100 pb-6">
        <div>
          ...
          <p className="text-5xl font-bold tabular-nums tracking-tight text-slate-900">
            {result.overall_score}
            {result.overall_provisional && (
              <span className="text-3xl font-semibold text-amber-600">*</span>
            )}
          </p>
          {result.overall_provisional && (
      ...
      <section className="rounded-lg border border-slate-100 bg-slate-50 p-4">
        ...
        <p className="text-xs text-slate-600">
          {MODEL_CONSENSUS_DESCRIPTION} All three models run in parallel on every
          score. Within 10 points = unanimous; two agree = majority; all diverge =
          review recommended. Use <strong>Show details</strong> per dimension for
          the full breakdown.
        </p>
