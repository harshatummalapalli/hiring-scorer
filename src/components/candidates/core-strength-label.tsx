...
import { isInvalidDisplaySkill } from "@/lib/candidates/candidate-identity-display";
...
export function CoreStrengthLabel({
  ...
  const safePrimary = isInvalidDisplaySkill(primary) ? null : primary?.trim() || null;
  const safeSecondary = isInvalidDisplaySkill(secondary) ? null : secondary?.trim() || null;
  const label = formatCoreStrengthLabel(safePrimary, safeSecondary);
  if (label) {
    return (
  ...
  if (!safePrimary && !safeSecondary && skills.length > 0) {
    return (
